...
from app.models import TuyenBay, db, ChuyenBay, Ghe, SanBay, HangMayBay, MayBay, SanBayTrungGian, HangVe
...
def find_flights(departure_time_str, route_code, seat_class, ticket_quantity):
    ...
    # Cộng thêm 4 giờ vào thời gian đầu vào
    target_time = departure_time + timedelta(hours=4)

    # Truy vấn các chuyến bay có thời gian đi sau thời gian đầu vào 4 giờ
    flights = db.session.query(ChuyenBay).filter(
        ChuyenBay.tuyen_bay == route_code,  # Lọc theo mã tuyến bay
        ChuyenBay.thoi_gian_di >= target_time  # Lọc các chuyến bay có thời gian đi sau 4 giờ
    ).all()
